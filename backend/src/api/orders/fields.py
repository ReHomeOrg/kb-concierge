...
# послезавтра, день недели, время ЧЧ:ММ, «N <месяца>», дата ДД.ММ(.ГГГГ).
_DATETIME_RE = re.compile(
    r"(?:сегодня|завтра|послезавтра"
    r"|(?:в\s+)?(?:понедельник|вторник|сред[уаы]|четверг|пятниц[уаы]|суббот[уаы]|воскресень[еяю])"
    r"|\d{1,2}[:.]\d{2}(?!\d)"
    r"|\d{1,2}\s+(?:янва|февра|март|апрел|ма[яй]|июн|июл|авгус|сентяб|октяб|нояб|декаб)\w*"
    r"|\d{1,2}\.\d{1,2}(?:\.\d{2,4})?)",
    re.IGNORECASE,
)
...
def extract_fields(category: str, masked_text: str) -> dict[str, str]:
    ...
    ремонта, грузчики/упаковка, дата/время); остальное добирается вопросами. Для REPAIR
    описание неисправности = сам текст обращения (всегда присутствует).
    """
    text = masked_text.lower()
    found: dict[str, str] = {}
    # Дата/время — для всех категорий с полем `datetime` (#2: мульти-поле из одной реплики).
    if "datetime" in required_keys(category):
        dt_match = _DATETIME_RE.search(masked_text)
